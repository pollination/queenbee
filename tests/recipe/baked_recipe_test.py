...
class TestFolder(BaseTestClass):
    ...
    def pytest_generate_tests(self, metafunc):
        ...
        invalid_recipe_paths = [recipe_path for recipe_path in invalid_recipe_paths if recipe_path.endswith('yaml')]
        ...
        if "folder" in metafunc.fixturenames:
            metafunc.parametrize('folder', folders, ids=[os.path.basename(folder) for folder in folders])
        ...
        if 'recipe' in metafunc.fixturenames:
            metafunc.parametrize('recipe', recipes, ids=[recipe.metadata.name for recipe in recipes])

        if 'invalid_recipe' in metafunc.fixturenames and 'error_message' not in metafunc.fixturenames:
        
            metafunc.parametrize('invalid_recipe', invalid_recipe_paths, ids=[os.path.basename(rec_path) for rec_path in invalid_recipe_paths])
        ...
        if 'invalid_recipe' in metafunc.fixturenames and 'error_message' in metafunc.fixturenames:
            ...
            metafunc.parametrize(
                ...
                ids=[os.path.basename(rec_path) for rec_path in invalid_recipe_paths],
            )
